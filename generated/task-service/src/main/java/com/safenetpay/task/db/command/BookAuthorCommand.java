...
public class BookAuthorCommand {
    ...
    * @return the bookAuthor command
    */
    public Future<BookAuthorList> bookAuthorGetSummaryListCommand(Long loginId, String sortExpression, String filterCondition, Long skip, Long pageSize) {
        LOGGER.info("info: bookAuthorGetSummaryListCommand - start");
        Promise<BookAuthorList> promise = Promise.promise();
        client
            ...
            .onSuccess(res -> {
                ...
                try{
                    ...
                    for (Row row : res) {
                        BookAuthor bookAuthor = createBookAuthor(row);
                        LOGGER.info("info: handle query bookAuthorGetSummaryListCommand result - ok");
                        bookAuthorList.add(bookAuthor);
                    }
                    ...
                    if (promise.tryComplete()) {
                      LOGGER.info("info: handle query bookAuthorGetSummaryListCommand result - no result");
                    }
                } catch(Exception e){
                    LOGGER.info("info: handle query bookAuthorGetSummaryListCommand result - no result");
                    promise.fail(e);
            ...
            .onFailure(ar -> {
                LOGGER.error("info: handle query bookAuthorGetSummaryListCommand result - failed");
                promise.fail(ar);
            });
        return promise.future();
    }
